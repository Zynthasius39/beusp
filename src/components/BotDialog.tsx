...
export default function BotDialog() {
  ...
  const [botInfo, setBotInfo] = useState<BotInfoJson>({
    botEmail: undefined,
    botTelegram: undefined,
  });
  ...
  const [telegramCode, setTelegramCode] = useState<number | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    timer.current = setInterval(() => {
      ...
      if (Math.round(verifyTimeout) % 8 === 0) getBotSubs();
    }, 1000);
  ...
  return (
    <>
            ...
            <ListItem
              ...
              secondaryAction={
                <IconButton
                  edge="end"
                  aria-label="unsubscribe"
                  onClick={() => handleUnsub("discord")}
                >
                  {subs?.discordWebhookUrl && <Close />}
                </IconButton>
              }
              disablePadding
            >
            ...
            <ListItem
              ...
              secondaryAction={
                <IconButton
                  edge="end"
                  aria-label="unsubscribe"
                  onClick={() => handleUnsub("telegram")}
                >
                  {subs?.telegramUserId && <Close />}
                </IconButton>
              }
              disablePadding
            >
      ...
      <Dialog open={useTelegram} onClose={handleClose}>
        ...
        <DialogContent>
          ...
          <Stack alignItems="center" gap={2} pt={4}>
            ...
            <Link
              href={`https://t.me/${botInfo.botTelegram}`}
              fontSize={{
                xs: 20,
                sm: 32,
              }}
      ...
      <Snackbar open={alert != undefined}>{alert}</Snackbar>
    </>
  );
}
