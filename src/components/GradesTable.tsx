        ...
        <TableContainer sx={{
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        {
                            ["courseCode", "courseName", "act1", "act2", "act3", "attendance", "iw", "final", "sum", "mark"]
                                .map(h => {
                        ...
                        sortedRows.map((course, inx) => {
                            ...
                            return (
                                <TableRow key={inx} sx={{ backgroundColor: inx % 2 === 0 ? 'background.paper' : 'inherit' }}>
                                    ...
                                    <TableCell sx={tableCellStyle}>{getGradeValue(courseG.act2)}</TableCell>
                                    {
                                        act3Enabled &&
                                        <TableCell sx={tableCellStyle}>{getGradeValue(courseG.act3)}</TableCell>
                                    }
                                    <TableCell sx={tableCellStyle}>{getGradeValue(courseG.attendance)}</TableCell>
                                    <TableCell sx={tableCellStyle}>{getGradeValue(courseG.iw)}</TableCell>
                                    <TableCell sx={tableCellStyle}>{getGradeValue(courseG.final)}</TableCell>
                                    <TableCell sx={tableCellStyle}>{courseG.sum !== -1 ? courseG.sum : calcGrade && getGradeValue(courseG.calcSum)}</TableCell>
                                    <TableCell>
                                            ...
                                            <Paper elevation={5} sx={{ borderRadius: "50%", width: 40 }}>
                                                ...
                                                >
                                                    {gradeToMark(courseG.sum)}
                                                    {
                                                        ...
                                                        <IconButton onClick={(e) => handleCalcClick(e, course)}>
                                                            <Calculate sx={{
