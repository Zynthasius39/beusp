...
export const gradeScale = (
  ...
  const act3_enabled = course.act3 !== undefined;
  const semEnabled = course.sem !== undefined;
  const courseG = Object.assign({}, course);
  if (courseG.act1 !== -1)
    courseG.act1 = parseFloat(
      (oldScale
        ? Math.round(
            (courseG.act1 / (act3_enabled || semEnabled ? 10 : 15)) * 100,
          )
        : round
          ? gradeRound(courseG.act1)
          : courseG.act1
      ).toFixed(2),
    );
  if (courseG.act2 !== -1)
    courseG.act2 = parseFloat(
      (oldScale
        ? Math.round(
            (courseG.act2 / (act3_enabled || semEnabled ? 10 : 15)) * 100,
          )
        : round
          ? gradeRound(courseG.act2)
          : course.act2
      ).toFixed(2),
    );
  ...
  if (courseG.iw !== -1)
    courseG.iw = parseFloat(
      (oldScale
        ? courseG.iw * 10
        : round
          ? gradeRound(course.iw)
          : courseG.iw
      ).toFixed(2),
    );
  if (courseG.final !== -1)
    courseG.final = parseFloat(
      (oldScale
        ? courseG.final * 2
        : round
          ? gradeRound(course.final)
          : courseG.final
      ).toFixed(2),
    );
  return courseG;
...
export const isValidDcWebhook = (url: string) => {
  return /https:\/\/discord.com\/api\/webhooks\/\d{19}\/[-_a-zA-Z0-9]{68}\/?/.test(
    url,
  );
};
...
export const pointsNeeded = (
  sum: number,
  category: "queen" | "rook" | "pawn",
): number => {
  switch (category) {
    case "queen":
      return 91 - sum;
...
export const thresholdColor = (
  ...
  let color = "primary";
  if (warning !== undefined && percent >= warning) color = "warning";
  if (critical !== undefined && percent >= critical) color = "error";
